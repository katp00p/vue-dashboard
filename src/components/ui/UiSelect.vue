...
const hasIconLeft = computed(
  () => props.showIconLeft && (!!selectedIconClass.value || !!selectedSi.value),
)
...
async function onSelect(idx) {
  ...
  if (!opt) return
  // Close first to avoid any parent handlers accidentally re-opening via global events.
  closeDropdown(false)
  // Emit after the DOM has applied the close, so the popup stays closed.
  await nextTick()
...
<template>
  <div class="relative">
    ...
    <button
      ref="triggerRef"
      type="button"
      class="relative w-full control flex items-center justify-between focus:outline-none focus:ring-2 focus:ring-[var(--ring)]"
      :class="hasIconLeft ? 'pl-9' : 'pl-3'"
    ...
    >
      ...
      <span
        v-if="hasIconLeft"
        class="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 flex items-center justify-center opacity-80"
        aria-hidden="true"
      >
        <SiIcon v-if="selectedSi" :name="selectedSi" class="h-4 w-4" />
        <i
          v-else-if="selectedIconClass"
          :class="selectedIconClass + ' text-[16px] leading-none fa-fw'"
        />
      </span>
    ...
    <div
      ...
      role="listbox"
      tabindex="-1"
      class="absolute left-0 right-0 mt-1 z-50 control-surface max-h-60 overflow-auto p-1 shadow-lg"
    ...
    >
      <div
        ...
        class="px-3 py-2 rounded text-slate-100 cursor-pointer flex items-center gap-2 justify-between hover:bg-[#565e66]"
        :class="[
          idx === activeIndex ? 'bg-[#565e66]' : '',
          modelValue === opt.value ? 'ring-1 ring-[#6a737d]' : '',
        ]"
        @mouseenter="activeIndex = idx"
        @click.stop.prevent="onSelect(idx)"
      >
