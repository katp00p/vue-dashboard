...
const addType = ref('list') // 'task' | 'list' | 'project'
const title = ref('')
const parentProjectId = ref(null) // for new List
const parentListId = ref(null) // for new Task
...
function submitAdd() {
  ...
  if (t === 'project') {
    ...
    store.addList(text, parentProjectId.value || null)
  } else {
    store.addTask(text, parentListId.value || null)
  }
  closeAdd()
}
...
<template>
  ...
  >
    ...
    <div class="scrollbar px-4 py-3 pb-4 overflow-auto min-h-0 flex-1 space-y-6">
      ...
      <section v-if="projects.length">
        ...
        <ul class="space-y-2">
          <li
            v-for="p in projects"
            :key="p.id"
            class="rounded-lg p-3 border border-accent/30 bg-accent/15"
          >
            <div class="flex items-center justify-between">
              <div class="text-slate-100 font-medium">{{ p.title }}</div>
      ...
      <section v-if="ungroupedLists.length">
        ...
        <ul class="space-y-2">
          <li
            ...
            :key="l.id"
            class="rounded-lg p-3 border border-white/10 bg-white/5"
          >
            <div class="flex items-center justify-between">
              <div class="text-slate-100 font-medium">{{ l.title }}</div>
              <div class="text-xs text-slate-300">{{ l.taskIds.length }} tasks</div>
            </div>
          </li>
        </ul>
      </section>
      ...
      <section v-if="ungroupedTasks.length">
        ...
        <ul class="space-y-2">
          <li v-for="t in ungroupedTasks" :key="t.id" class="flex items-start gap-2">
            <input
              class="mt-1"
              type="checkbox"
              :checked="t.done"
            ...
            />
            <p class="text-slate-100" :class="t.done ? 'line-through text-slate-400' : ''">
              {{ t.title }}
            </p>
          </li>
        </ul>
  ...
  <!-- Add Entity Modal (with 3-button switch) -->
          ...
          >
            ...
            >
              ...
              <div class="border-b border-white/10 px-5 pt-[1px] pb-2 -mx-5">
                <div class="flex items-center justify-between px-4">
                  <DialogTitle class="card-title text-slate-100 font-semibold leading-tight">
                    Add
                    {{
                      addType === 'project' ? 'Project' : addType === 'list' ? 'Task List' : 'Task'
                    }}
                  </DialogTitle>
                  <button
                    type="button"
              ...
              <div class="px-6 py-5 space-y-4">
                ...
                <!-- Title -->
                <div>
                  <label for="entity-title" class="block text-sm text-slate-300 mb-1">Title</label>
                  <input
                    id="entity-title"
                    ref="initialFocusRef"
                    type="text"
                ...
                <div v-if="addType === 'list'">
                  <label class="block text-sm text-slate-300 mb-1">Add to Project (optional)</label>
                  <select
                    class="w-full rounded-lg bg-white/5 border border-white/10 px-3 py-2 text-slate-100 focus:outline-none focus:ring-2 focus:ring-accent/40"
