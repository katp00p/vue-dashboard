                    ...
                    <section class="space-y-3">
                        ...
                        <div class="flex items-center justify-between">
                            <p class="text-sm text-slate-400">
                                Drag icons in the left column to change order. Click an icon to edit its settings in
                                the right panel.
                            </p>

                            <button type="button" disabled class="px-3 py-2 rounded-md bg-white/10 text-slate-100 border border-white/15 hover:bg-white/15 focus:outline-none focus:ring-2 focus:ring-sky-400/40 disabled:opacity-50 disabled:cursor-not-allowed text-sm" title="Maximum of 39 icons reached">
                                Add Icon
                            </button>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="bg-white/5 rounded-md p-4 space-y-2">
                                ...
                                <div class="flex flex-col gap-2 max-h-[50vh] overflow-y-auto pr-1">
                                    <button v-for="item in shortcuts" :key="item.id" type="button" class="flex items-center gap-3 p-2 bg-white/10 rounded cursor-move text-left hover:bg-white/15 focus:outline-none" :class="activeIconId === item.id ? 'ring-2 ring-[#949ba1]' : ''" @click="activeIconId = item.id">
                            ...
                            <div class="bg-white/5 rounded-md p-4 flex flex-col">
                                ...
                                <div v-if="activeIcon" class="flex flex-col gap-4 flex-1">
                                    ...
                                    <div>
                                        ...
                                        <input v-model="activeIcon.label" type="text" class="control w-full text-sm rounded-md bg-[#4b535a] text-slate-100 border border-white/10 focus:outline-none focus:ring-2 focus:ring-sky-400/40 focus:border-white/20" placeholder="e.g., Facebook" autocomplete="off" />
                                    </div>

                                    <!-- Link -->
                                    <div>
                                        <label class="block text-xs font-medium text-slate-300 mb-1">Link</label>
                                        <input v-model="activeIcon.href" type="url" class="control w-full text-sm rounded-md bg-[#4b535a] text-slate-100 border border-white/10 focus:outline-none focus:ring-2 focus:ring-sky-400/40 focus:border-white/20" placeholder="https://facebook.com" autocomplete="off" />
                                    </div>

                                    <!-- Icon Class -->
                                    <div>
                                        <label class="block text-xs font-medium text-slate-300 mb-1">Icon Class</label>
                                        <input v-model="activeIcon.icon" type="text" class="control w-full text-sm rounded-md bg-[#4b535a] text-slate-100 border border-white/10 focus:outline-none focus:ring-2 focus:ring-sky-400/40 focus:border-white/20" placeholder="fa-brands fa-facebook" autocomplete="off" />
                                    </div>
                                    ...
                                    <div class="mt-auto rounded-md border border-white/10 bg-white/5 p-4">
                                        ...
                                        <div class="mt-2 flex justify-end gap-2">
                                            <template v-if="confirmDelete">
                                                <button type="button" class="px-3 py-2 rounded-md bg-gray-600 text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-400/40 text-sm" @click="confirmDelete = false">
                                                    No
                                                </button>
                                                <button type="button" class="px-3 py-2 rounded-md bg-[#553744] text-slate-100 border border-white/10 hover:bg-[#6a4756] focus:outline-none focus:ring-2 focus:ring-sky-400/40 text-sm" @click="/* later: real delete */ confirmDelete = false">
                                                    Yes
                                                </button>
                                            </template>
                                            <template v-else>
                                                <button type="button" class="px-3 py-2 rounded-md bg-[#553744] text-slate-100 border border-white/10 hover:bg-[#6a4756] focus:outline-none focus:ring-2 focus:ring-sky-400/40 text-sm" @click="confirmDelete = true">
                                                    Delete
                                                </button>
                                            </template>
                                        </div>
